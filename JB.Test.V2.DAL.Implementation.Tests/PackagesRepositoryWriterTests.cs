...
{
	public sealed class PackagesRepositoryWriterTests : RepositoryTestsBase
	{	
		[Fact]
		public async Task AddPackageTest()
		{
			...
			var package = await factory.CreateFromFileAsync(
				Path.Combine(TestsConstants.TestInputPath, "Moq.4.8.2.nupkg"), CancellationToken.None);

			var repo = Container.Resolve<IPackagesRepositoryWriter>();
			await repo.AddPackageAsync(package, CancellationToken.None);

			var path = Path.Combine(TestsConstants.OutPath, $"{package.Id}.{package.Version}{Constants.NugetPackageExtension}");
		...
		{
			var factory = Container.Resolve<IPackagesFactory>();
			var package = await factory.CreateFromFileAsync(Path.Combine(TestsConstants.TestInputPath, "Moq.4.8.2.nupkg"),
			...
			var exception = await Assert.ThrowsAsync<InvalidOperationException>(async()=> 
				await repo.AddPackageAsync(packageMock.Object, CancellationToken.None));
			...
			Assert.False(File.Exists(Path.Combine(TestsConstants.OutPath, package.BuildFileName())));
		}
		...
		public async Task SyncRepositoryTest()
		{
			...
			var inputPackage = await factory.CreateFromFileAsync(
				Path.Combine(TestsConstants.InputPath, "AutoFixture.4.8.0.nupkg"),
				CancellationToken.None);
			...
			var path = Path.Combine(TestsConstants.OutPath, package.BuildFileName());
			Assert.True(File.Exists(path));
			...
			Assert.NotEmpty(Directory.EnumerateFiles(TestsConstants.InputPath));
			...
			Assert.Empty(Directory.EnumerateFiles(TestsConstants.InputPath));
			...
			var addedPath = Path.Combine(TestsConstants.OutPath, inputPackage.BuildFileName());
